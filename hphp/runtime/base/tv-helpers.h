...
inline TypedValue* tvBox(TypedValue* tv) {
	...
	assert(tv->m_type != KindOfRef);
	//ameijer
	//move taint tags to the value being pointed to
	//OPTIMIZATION OPPORTUNITY - change refdata constructor to take tags in to save ops here
	//will likely result in massive compiler distress though...
	uint64_t tags;
	bool fixTags = false;
	if(tv->tTags[0] == VALID_TAG){
		tags = tv->tTags[1];
		fixTags = true;
	}

	//regardless of whether this tag was valid or not, scrub it to prevent over-tainting
	tv->tTags[0] = 0;
	tv->tTags[1] = 0;
...
inline void tvUnbox(TypedValue* tv) {
	...
	if(innerCell->tTags[0] == VALID_TAG){
		tv->tTags[0] = VALID_TAG;
		tv->tTags[1] = innerCell->tTags[1];
	} else {
		//otherwise we will scrub 'em
		tv->tTags[0] = 0;
		tv->tTags[1] = 0;
	}
...
inline void tvCopy(const TypedValue& fr, TypedValue& to) {
	...
	//ameijer
	//copy taints
	to.tTags[0] = fr.tTags[0];
	to.tTags[1] = fr.tTags[1];
