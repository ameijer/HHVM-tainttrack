...
struct TypedValue {
	...
	//ameijer
	////////////////////////////////////////////////////////////////////TAINT TRACKING ameijer
#define VALID_TAG 0xDEADBEFADED

	void checkandsetvalidtag();
	...
	uint64_t setttags(uint64_t tags);
	...
	//zero out the array if it has not yet been initialized
	void checkinit(){
		if(this->tTags[2] != 1){
			//then the array has not been initialized
...
private:
	static void assertions() {
		//ameijer
		//static_assert(sizeof(TypedValueAux) <= 16, "don't add big things to AuxUnion");
	}
};
