...
struct Variant : private TypedValue {
	...
	/////////////TAINT TRACKING STUFF ameijer//////////////////////////////////////

	uint64_t getTaintArrayReadOnly(){
		return this->TypedValue::getTaintArrayReadOnly();
	}

	//adds in tags using OR
		uint64_t addttags(uint64_t toAdd){
			return this->TypedValue::addttags(toAdd);

		}
	uint64_t settainttag(uint8_t index){

		return this->TypedValue::settainttag(index);
	}

	uint64_t cleartainttag(uint8_t index){
		return this->TypedValue::cleartainttag(index);
	}

	uint64_t setallttags(){
		return this->TypedValue::setallttags();
	}

	uint64_t clearallttags(){
		return this->TypedValue::clearallttags();
	}

	//Added for taint tracking ameijer
	uint64_t settag(uint64_t source){

		return this->TypedValue::setttags(source);
	}
