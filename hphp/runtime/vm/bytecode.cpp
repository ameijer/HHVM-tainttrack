...
OPTBLD_INLINE bool ExecutionContext::memberHelperPre(
  ...
  while (vec < pc) {
    ...
    ratchetRefs(result, tvRef, tvRef2); //possible point for tag propagation
    //ameijer
    result->addttags(base->getTaintArrayReadOnly());
    ...
    base = result;
    //ameijer
     // memset(result, 0, sizeof(TypedValue));
  }
...
OPTBLD_INLINE void ExecutionContext::implCellBinOp(IOP_ARGS, Op op) {
  ...
  auto result = op(*c2, *c1);
  //ameijer
  //OPTIMIZATION OPPORTUNITY
  if(c1->tTags[0] != VALID_TAG){
	  //if this isn't a valid tag, make it so that it has no effect on the taint tag propagation
	  c1->tTags[1] &= 0;
  }
  if(c2->tTags[0] != VALID_TAG){
  	  //if this isn't a valid tag, make it so that it has no effect on the taint tag propagation
  	  c2->tTags[1] &= 0;
    }

  result.tTags[1] =  c1->tTags[1] | c2->tTags[1];
  result.tTags[0] =  VALID_TAG;
  tvRefcountedDecRefCell(c2);
  *c2 = (const HPHP::TypedValue) result;
  vmStack().popC();
}
...
OPTBLD_INLINE void ExecutionContext::iopSetM(IOP_ARGS) {
  ...
  if (!setHelperPre<false, true, false, false, 1,
    ...
    //ameijer
      //add the taint from the added item to the member it is added to
      base->addttags(c1->getTaintArrayReadOnly());
  }
